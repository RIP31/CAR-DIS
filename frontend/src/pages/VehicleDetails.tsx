      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 space-y-8 min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="h-[450px] w-full rounded-3xl overflow-hidden border border-white/5 relative bg-[#121418]/20">
              <img
                src={activeImage}
            ...
            <div className="grid grid-cols-4 gap-4">
              {thumbnails.map((thumb, idx) => (
                <button
                  ...
                  className={`h-20 rounded-xl overflow-hidden border-2 transition-all bg-white/2 cursor-pointer ${
                    activeImage === thumb ? 'border-teal-400 scale-98 shadow-md' : 'border-transparent hover:border-white/10'
                  }`}
                >
                  <img
                    src={thumb}
            ...
            <div className="glass p-8 rounded-3xl border border-white/5 space-y-6 bg-[#0d0e12]/60">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-widest bg-teal-500/10 border border-teal-500/20 text-teal-400 px-3 py-1 rounded-full">
                    {vehicle.category}
                  </span>
                  
                  <span className={`text-xs font-semibold ${isOutOfStock ? 'text-rose-400' : 'text-slate-400'}`}>
                    {isOutOfStock ? 'Sold Out' : `${vehicle.quantity} Available`}
                  </span>
                </div>

                <h1 className="text-3xl font-extrabold text-white tracking-tight leading-tight pt-1">
                  {vehicle.make} {vehicle.model}
                </h1>
                
                <p className="text-3xl font-extrabold text-teal-400 font-['Outfit'] pt-1">
                  ${vehicle.price.toLocaleString()}
                </p>
              </div>

              {/* Specifications Table */}
              <div className="border-t border-white/5 pt-5 space-y-4 text-sm font-semibold">
                ...
                <div className="space-y-3.5 pt-1">
                  <div className="flex justify-between items-center py-0.5">
                    ...
                    <span className="text-white flex items-center gap-1.5">
                      <Calendar className="h-4 w-4 text-teal-400/50" />
                      {vehicle.year}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-0.5">
                    ...
                    <span className="text-white flex items-center gap-1.5">
                      <Fuel className="h-4 w-4 text-teal-400/50" />
                      {vehicle.fuel_type}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-0.5">
                    ...
                    <span className="text-white flex items-center gap-1.5">
                      <Layers className="h-4 w-4 text-teal-400/50" />
                      {vehicle.category}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className={`w-full py-4 text-xs font-bold uppercase tracking-wider rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer border-none ${
                    isOutOfStock
                      ...
                      : 'bg-white text-black hover:bg-slate-200 shadow-lg shadow-white/5 font-extrabold'
                  }`}
                >
        ...
        {/* Description Section */}
        <section className="glass p-8 rounded-3xl border border-white/5 bg-[#0d0e12]/30 space-y-4">
          <h3 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">
            <FileText className="h-5 w-5 text-teal-400" />
            Detailed Description
          </h3>
          <p className="text-slate-400 leading-relaxed font-normal text-sm max-w-4xl">
            {vehicle.description ||
